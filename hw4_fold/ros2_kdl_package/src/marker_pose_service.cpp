...
public:
  MarkerPoseService() : Node("marker_pose_service")
  {
    ...
    // Crea il client verso gzition
    client_ = this->create_client<ros_gz_interfaces::srv::SetEntityPose>( "/world/pizzeria_world/set_pose");


    // Aspetta che il servizio gzition sia disponibile
...
private:
  ...
  {
    // Prepara la richiesta per gzition
    auto gz_req = std::make_shared<ros_gz_interfaces::srv::SetEntityPose::Request>();
    gz_req->pose.position.x = request->x;
    gz_req->pose.position.y = request->y;
    gz_req->pose.position.z = request->z;
    gz_req->entity.name = "my_pizza";
    gz_req->entity.type = ros_gz_interfaces::msg::Entity::MODEL;


    // Invia la richiesta
    auto future = client_->async_send_request(gz_req);
